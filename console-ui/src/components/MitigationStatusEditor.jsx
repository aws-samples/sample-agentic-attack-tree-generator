 ...
 *
 * @param {Object}   props
 * @param {string}   props.mitigationKey  — the canonical mitigation_text key the
 *                                          backend uses to look up the override
 * @param {string}   props.appId
 * @param {string}   props.versionId
 * @param {string|null} props.status      — current override status, or null
 * @param {string}   props.comment        — current override comment
 * @param {Function} props.onSaved        — called with ({status, comment}) after a
 *                                          successful save; parent updates local state
 * @param {Function} props.onCleared      — called after a successful clear
 */
export default function MitigationStatusEditor({
  ...
  const selectedOption = STATUS_OPTIONS.find((o) => o.value === draftStatus) || STATUS_OPTIONS[0];
    ...
    <div style={{ minWidth: 140 }}>
      ...
      {info ? (
        <SpaceBetween size="xxs">
          ...
          {comment && (
            <Box variant="small" color="text-body-secondary">{comment}</Box>
          )}
          <Button
            iconName="edit"
            variant="inline-link"
