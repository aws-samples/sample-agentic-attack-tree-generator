...
/**
 * AppOverviewView — TS/Next port of console-ui's pages/AppOverviewPage.jsx.
 *
 * The v2 landing page for an application: header + rename, business-context
 * panel, project-path edit, latest threat model card, version history table,
 * report export, and a danger-zone delete. Rendered by the dynamic-segment
 * server wrapper (`page.tsx`, generateStaticParams: []); reads `appId` via
 * `useParams()`.
 */
...
import type { Application, BusinessContext, VersionSummary } from '@threatforest/types';
...
/**
 * The overview page mixes a real persistent `Application` record with a
 * hand-built stub for imported apps that lack one (extra `description` /
 * `imported` / `imported_from` fields). This loose superset captures every
 * field the page reads regardless of which path populated it.
 */
...
/** Scope of a ThreatForest Report export, or null when the modal is closed. */
type ReportScope = 'version' | 'full' | null;
...
function isLiveVersion(version: VersionSummary | null | undefined): boolean {
  return Boolean(
    version &&
...
function isAbandonedVersion(version: VersionSummary | null | undefined): boolean {
  return Boolean(
    version &&
...
export default function AppOverviewView() {
  ...
  const [app, setApp] = useState<AppOverviewState | null>(null);
  ...
  const [versionDeleteTarget, setVersionDeleteTarget] = useState<VersionSummary | null>(null);
  ...
  const [reportPrompt, setReportPrompt] = useState<ReportScope>(null);
    ...
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        const [appResult, versionsData] = await Promise.all([
          getApplication(appId).catch((err: unknown) => ({ __error: err })),
          getApplicationVersions(appId).catch(() => ({ versions: [] as VersionSummary[] })),
        ]);
        if (signal?.aborted) return;

        if (appResult && '__error' in appResult) {
          // Try the folder-derived listing for an imported-app summary.
          let stub: AppOverviewState | null = null;
          try {
            const list = await getApplications();
            const found = (list?.applications || []).find((a) => a.id === appId) || null;
            if (found) {
              stub = {
                id: found.id,
                name: found.name,
                ...
                imported_from: found.imported_from,
                project_path: '',
                ...
                business_context: found.business_context || null,
              };
            }
          } catch {
          ...
          setApp(appResult as Application as AppOverviewState);
        }
  ...
  const handleBusinessContextUpdated = (updatedApp: Application) => {
    setApp(updatedApp as AppOverviewState);
  };
  ...
  const handleRenameSave = async () => {
    ...
    try {
      ...
      setApp(updated as AppOverviewState);
      setRenaming(false);
    } catch (err) {
      setRenameError((err as Error).message || 'Failed to rename application.');
    } finally {
  ...
  const handlePathSave = async () => {
    ...
    try {
      ...
      setPathError((err as Error).message || 'Failed to update project path.');
    } finally {
  ...
  const handleReportExport = async () => {
    ...
    try {
      await downloadThreatforestReport({
        ...
        versionId: reportPrompt === 'version' && latestVersion ? latestVersion.id : undefined,
        includeScannerContext: reportIncludeContext,
      });
      setReportPrompt(null);
    } catch (err) {
      setExportError((err as Error).message || 'Failed to export ThreatForest Report.');
    } finally {
  ...
  const handleVersionDelete = async () => {
    ...
    try {
      ...
      setVersionDeleteError((err as Error).message || 'Failed to delete threat model.');
    } finally {
  ...
  const versionColumnDefinitions: ReadonlyArray<TableProps.ColumnDefinition<VersionSummary>> = [
    {
      id: 'version_id',
      header: 'Threat model',
      cell: (item) => {
        ...
        if (abandoned) {
          // Non-clickable: there is nothing to render for an abandoned run.
          // Keep the label so the user can still correlate with timestamps.
          return <Box color="text-status-inactive">{displayLabel}</Box>;
        }
        // Always use the explicit version id, even for the "latest" row. The
        // /versions/latest alias would resolve server-side to whatever's most
        // recent on disk at click-time, which can shift if a new run lands
    ...
    },
    {
      ...
      header: 'Actions',
      cell: (item) => {
        // Don't let the user delete a live run out from under an in-flight
        // pipeline — the backend will 400 anyway, but hiding the control
        // entirely avoids confusion. Likewise, exports need a completed
        // dashboard to read from, so disable for live and abandoned rows.
        const live = isLiveVersion(item);
        const abandoned = isAbandonedVersion(item);
        return (
  ...
  return (
    ...
    >
      <SpaceBetween size="l">
          ...
          actions={
            <SpaceBetween direction="horizontal" size="xs">
              ...
              {!isImported && (
                <Button
                  variant="primary"
                  onClick={() => router.push(`/applications/${appId}/runs/new`)}
        ...
        {isImported && (
          <Alert type="info" header="Imported application" data-testid="imported-banner">
            This application was imported from{' '}
            <strong>{app?.imported_from || 'another ThreatForest install'}</strong>. The source
            code is not available here, so re-running and editing the business context are
            disabled. You can still browse threat models, edit mitigation status, and export.
          </Alert>
        )}
                  ...
                  <Link
                    onFollow={(e) => {
                      ...
                      if (isLiveVersion(latestVersion)) {
                        router.push(`/runs/${latestVersion.run_id}/progress`);
                      } else {
                        ...
                        router.push(`/applications/${appId}/versions/${latestVersion.id}`);
                      }
                    }}
                  >
                    {latestVersion.display_name || latestVersion.id}
          ...
          <Table
            ...
            empty={
              <Box textAlign="center" color="inherit" padding="l">
                <SpaceBetween size="m">
                  <b>No threat models yet</b>
                  <Box color="inherit">Run an analysis to create the first threat model.</Box>
                </SpaceBetween>
              </Box>
            }
          />
        ...
        >
          <SpaceBetween size="s">
            ...
            <Box variant="p">
              Permanently removes this application record and its threat model history from the
        ...
        >
          <SpaceBetween size="s">
            {versionDeleteError && <Alert type="error">{versionDeleteError}</Alert>}
            <Box variant="p">
              Are you sure you want to delete{' '}
              <strong>{versionDeleteTarget?.display_name || versionDeleteTarget?.id}</strong>? The
              threat statements, attack trees, and dashboard for this run will be removed from disk.
        ...
        >
          <SpaceBetween size="m">
            <Box variant="p">
              ...
              as a <code>.tfreport</code> file. Recipients can drop it into their{' '}
              <code>.threatforest/imports/</code> folder to load this application into their own
              ThreatForest install. Imported applications are read-only.
            </Box>
            <Toggle
            ...
            <Box variant="small" color="text-body-secondary">
              On by default for intra-team handoff. Turn off when sharing outside your team to
              redact source-code references.
            </Box>
          </SpaceBetween>
        </Modal>
      </SpaceBetween>
