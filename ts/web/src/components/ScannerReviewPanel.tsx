...
/**
 * Scanner findings as surfaced to the review modal. The server emits snake_case
 * fields; `cloud_provider` / `tech_stack` arrive as comma-joined strings while
 * the list fields arrive as arrays.
 */
export interface ScannerData {
  ...
  cia_priority?: string[];
  main_cia_risk?: string;
  /** Derived display tokens precomputed by the parent. */
  _cloudTokens?: string[];
  _techTokens?: string[];
}

/** Edits emitted back to the parent when the user saves the modal. */
export interface ScannerReviewEdits {
  industry: string;
...
function TokenField({ label, items, setItems, placeholder, info }: TokenFieldProps) {
...
export function BadgeList({ items }: { items?: string[] | null }) {
...
export interface ScannerReviewEditModalProps {
  visible: boolean;
  scannerData?: ScannerData;
  onSubmit: (edits: ScannerReviewEdits) => void;
  onDismiss: () => void;
}
 ...
 */
export default function ScannerReviewEditModal({
  visible,
  scannerData = {},
  onSubmit,
  onDismiss,
}: ScannerReviewEditModalProps) {
  const [industry, setIndustry] = useState(scannerData.industry || '');
  const [cloudProviders, setCloudProviders] = useState<string[]>(toTokens(scannerData.cloud_provider));
  const [techStack, setTechStack] = useState<string[]>(toTokens(scannerData.tech_stack));
  const [services, setServices] = useState<string[]>(scannerData.services || []);
  const [authMechanisms, setAuthMechanisms] = useState<string[]>(scannerData.auth_mechanisms || []);
  const [compliance, setCompliance] = useState<string[]>(scannerData.compliance_requirements || []);
