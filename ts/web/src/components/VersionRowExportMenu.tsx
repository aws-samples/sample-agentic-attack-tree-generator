...
import type { VersionSummary } from '@threatforest/types';
import {
  exportCustomPdf,
...
const EXPORT_ITEMS: ButtonDropdownProps.Items = [
  { id: 'customise', text: 'Customise export...' },
  { id: 'tfreport', text: 'ThreatForest Report (.tfreport)' },
];

export interface VersionRowExportMenuProps {
  appId: string;
  appName?: string | null;
...
export default function VersionRowExportMenu({
  ...
  disabled = false,
  onError,
}: VersionRowExportMenuProps) {
  const [busy, setBusy] = useState(false);
  const [customiseOpen, setCustomiseOpen] = useState(false);
  const [customiseError, setCustomiseError] = useState<string | null>(null);
  const [data, setData] = useState<ThreatModelSummary | null>(null); // Cached /data response.

  // Cache the merged /data response per version so re-opens don't refetch.
  const cacheRef = useRef<ThreatModelSummary | null>(null);

  async function loadData(): Promise<ThreatModelSummary> {
    ...
    const json = (await response.json()) as ThreatModelSummary;
    cacheRef.current = json;
    return json;
  }

  const handleClick: ButtonDropdownProps['onItemClick'] = async ({ detail }) => {
    ...
    if (detail.id === 'tfreport') {
      ...
      try {
        ...
        if (onError) onError((err as Error).message || 'Export failed.');
      } finally {
        setBusy(false);
      }
      return;
    ...
    if (detail.id === 'customise') {
      ...
      try {
        ...
        setCustomiseError((err as Error).message || 'Failed to load version data.');
      } finally {
        setBusy(false);
      }
    }
  ...
  async function handleCustomiseConfirm({ sections, format }: CustomiseExportConfirm) {
    if (!data) {
    ...
    try {
      ...
      setCustomiseError((err as Error).message || 'Failed to generate export.');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <>
      <ButtonDropdown
        ...
        // Cloudscape ButtonDropdown has no 'inline-link' variant; the legacy
        // value was ignored at runtime (rendered as the default 'normal'), so
        // we drop it to keep behaviour identical while satisfying the types.
        disabled={disabled}
