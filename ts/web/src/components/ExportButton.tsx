...
function buildExportItems({
  ...
  fullAppExport: boolean;
}): ButtonDropdownProps.Items {
  const reportItems: ButtonDropdownProps.ItemOrGroup[] = [
    { id: 'report-version', text: 'This version only' },
  ];
  if (fullAppExport) {
...
export interface ExportButtonProps {
  summaryData?: ThreatModelSummary | null;
  appId?: string;
  versionId?: string;
  appName?: string | null;
...
/** ThreatForest Report modal mode. */
type ReportMode = 'report-version' | 'report-full-app';
...
export default function ExportButton({
  ...
  const [reportPrompt, setReportPrompt] = useState<{ mode: ReportMode } | null>(null);
  const [includeScannerContext, setIncludeScannerContext] = useState(true);

  const handleClick: ButtonDropdownProps['onItemClick'] = ({ detail }) => {
    setError(null);
  ...
  const fname = (scope: string | null, extension: string) =>
    buildExportFilename({
      appId: appId ?? '',
      versionId: versionId ?? '',
      appName,
  ...
  async function handleReportConfirm() {
    ...
    try {
      ...
      setError((err as Error).message || 'Failed to export ThreatForest report.');
    } finally {
      setBusy(false);
    }
  }
