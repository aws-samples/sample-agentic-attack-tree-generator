...
import type { CiaObjectiveT } from '@threatforest/types';
...
const OBJECTIVE_INFO: Record<
  CiaObjectiveT,
  { label: string; description: string; color: BadgeProps['color'] }
> = {
  confidentiality: {
...
function SortableRow({ id, rank }: SortableRowProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export interface CiaPriorityListProps {
  /** current ordering (length 3); index 0 is most important */
  value: CiaObjectiveT[];
  /** receives the new ordering */
  onChange: (next: CiaObjectiveT[]) => void;
}

/**
 * Drag-to-rank list of the three CIA objectives. Index 0 is most important.
 */
export default function CiaPriorityList({ value, onChange }: CiaPriorityListProps) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const oldIndex = value.indexOf(active.id as CiaObjectiveT);
    const newIndex = value.indexOf(over.id as CiaObjectiveT);
