 ...
 * `src/threatforest/agents/scanner/prompt.md` into a co-located string constant
 * (rather than readFileSync at runtime) so the engine package is self-contained
 * when bundled/published — no .md asset resolution needed. Matches the convention
 * used by `mitigation.prompt.ts`. Keep in sync with the Python prompt.md
 * byte-for-byte (backticks are escaped; the text is otherwise unchanged).
 */
export const SCANNER_SYSTEM_PROMPT = `# Scanner Agent System Prompt
