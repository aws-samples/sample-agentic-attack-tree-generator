...
Provide ONLY the 1-2 word title, nothing else."""
    ...
    def _process_threatcomposer_file(self, threat_file_path: str) -> List[Dict[str, Any]]:
        """Process ThreatComposer file using pure Python
        ...
        """
        try:
            self.logger.debug(f"Extracting threats using Python parser...")
            
            # Read and parse the ThreatComposer JSON file
            with open(threat_file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            # Extract threat data using pure Python
            threat_data = self._extract_threatcomposer_data(data)
    ...
    def _extract_threatcomposer_data(self, data: Dict) -> Dict[str, Any]:
        """Extract and structure ThreatComposer data
        
        Args:
            data: Raw ThreatComposer JSON data
            
        Returns:
            Structured threat data
        """
        def get_priority(threat: Dict) -> str:
            """Extract priority from threat metadata"""
            if 'metadata' in threat:
                for meta in threat.get('metadata', []):
                    if meta.get('key') == 'Priority':
                        return meta.get('value', 'Medium')
            return threat.get('priority', 'Medium')
        ...
        if 'applicationInfo' in data:
            result['application_context'] = {
                'name': data['applicationInfo'].get('name'),
        ...
        for threat in threats:
            priority = get_priority(threat).lower()
