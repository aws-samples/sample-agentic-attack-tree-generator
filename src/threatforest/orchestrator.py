...
class ThreatForestOrchestrator:
    ...
    def execute_workflow(self) -> Dict[str, Any]:
        ...
        # Start a tracing session — all agent traces share this session_id
        self.tracing_context.start_session(session_id)
        ...
        try:
            ...
            # Step 2: Context analysis — own trace for annotation queue
            if not self.state.context_complete:
                self.state.advance_to(WorkflowStage.CONTEXT_ANALYSIS)
                
                with self.tracing_context.session_trace(
                    name="context_analysis",
                    metadata=base_metadata,
                    tags=["trace_type:context_analysis"]
                ) as trace:
                    ...
                    trace.set_output({"context_files": context_result})
            ...
            # Step 3: Information extraction — own trace for annotation queue
            if not self.state.extraction_complete:
                ...
                with self.tracing_context.session_trace(
                    name="threat_statement_generation",
                    metadata=base_metadata,
                    tags=["trace_type:threat_statement"]
                ) as trace:
                    with self.tracing_context.span("threat_statement_generation") as span:
                        ...
                        extraction_result = self.extraction_tool.run(
                            ...
                            bedrock_model=self.config.bedrock_model,
                            aws_profile=self.config.aws_profile,
                            threat_file_path=self.config.threat_model_path
                        )
                        ...
                        span.set_metadata({"model_id": self.config.bedrock_model})
            ...
            # Step 4.5: TTC Mapping — own trace for annotation queue
            ...
            # Step 5: Generate summary — own trace for annotation queue
            ...
            self.tracing_context.end_session()
