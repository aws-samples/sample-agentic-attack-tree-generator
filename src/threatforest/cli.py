...
with _loading_console.status("[bold cyan]🌳 Initializing ThreatForest...", spinner="dots"):
    from threatforest.modules.cli import CLIWizard, CLIDisplay, WorkflowRunner
    from threatforest.config import config, ROOT_DIR
            ...
            if output_directory:
                ...
                if dashboard_path:
                    ...
                    try:
                        ...
                        console.print(f"   Dashboard: {abs_path}")
                        console.print(f"   [dim]If browser doesn't open automatically, run: {open_cmd}[/dim]\n")
                    ...
                    logger.warning(f"Dashboard HTML not found in output directory: {output_directory}")
                    console.print(f"\n📁 [bold cyan]Output Directory:[/bold cyan] {output_directory}")
                    console.print("   [yellow]Dashboard not found - check if attack trees were generated[/yellow]\n")
            else:
